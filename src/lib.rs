...
pub fn decrypt(input: &mut State, key: &str) {
    let tmp = hex::decode(key).unwrap();
    let mut key: [u8; 32] = [0; 32];
    key.copy_from_slice(&tmp[..]);
    let w: KeySchedule = key_expansion(key);
...
fn mix_column(c: Word) -> Word {
    ...
    let s0 = GMUL2[b0i] ^ GMUL3[b1i] ^ b2 ^ b3;
    let s1 = b0 ^ GMUL2[b1i] ^ GMUL3[b2i] ^ b3;
    let s2 = b0 ^ b1 ^ GMUL2[b2i] ^ GMUL3[b3i];
    let s3 = GMUL3[b0i] ^ b1 ^ b2 ^ GMUL2[b3i];
    ((s0 as u32) << 24 | (s1 as u32) << 16 | (s2 as u32) << 8 | s3 as u32).into()
}
...
fn inv_mix_column(c: Word) -> Word {
    let b0 = (c >> 24 & 0xff) as usize;
    let b1 = (c >> 16 & 0xff) as usize;
    let b2 = (c >> 8 & 0xff) as usize;
    let b3 = (c & 0xff) as usize;
    // https://en.wikipedia.org/wiki/Rijndael_MixColumns
    let s0 = GMUL14[b0] ^ GMUL11[b1] ^ GMUL13[b2] ^ GMUL9[b3];
    let s1 = GMUL9[b0] ^ GMUL14[b1] ^ GMUL11[b2] ^ GMUL13[b3];
    let s2 = GMUL13[b0] ^ GMUL9[b1] ^ GMUL14[b2] ^ GMUL11[b3];
    let s3 = GMUL11[b0] ^ GMUL13[b1] ^ GMUL9[b2] ^ GMUL14[b3];
    ((s0 as u32) << 24 | (s1 as u32) << 16 | (s2 as u32) << 8 | s3 as u32).into()
}
...
mod tests {
    ...
    static KEY_EXPANSION_KEY: [u8; 32] = [
    ...
    fn shift_rows_test() {
        ...
        let shifted = [0x632fafa2, 0xeb93c720, 0x9f92abcb, 0xa0c0302b];
    ...
    fn sub_bytes_test() {
        ...
        let original = state.clone();
        let subbed = [0x63f2c9fa, 0xc9f2c963, 0xfe637d82, 0x3026d4d4];
        sub_bytes(&mut state);
        assert_eq!(state, subbed);
    ...
    fn mix_columns_test() {
        ...
        let mut columns = [0xdb135345, 0xf20a225c, 0x01010101, 0xc6c6c6c6];
        let original = columns.clone();
        let mixed = [0x8e4da1bc, 0x9fdc589d, 0x01010101, 0xc6c6c6c6];
        mix_columns(&mut columns);
        assert_eq!(columns, mixed);

        inv_mix_columns(&mut columns);
    ...
    fn decrypt_test() {
        ...
        let mut ciphertext = hex_to_u32arr(ciphertext);
    ...
    fn inv_debug() {
        ...
        let ciphertext = "8ea2b7ca516745bfeafc49904b496089";
        let mut state = hex_to_u32arr(ciphertext);

        let test_table = init_inv_debug_table();

        println!("round 0");
        assert_eq!(state, test_table[0][0]);
        assert_eq!(&w[NR * NB..(NR + 1) * NB], test_table[0][1]);
        add_round_key(&mut state, &w[NR * NB..(NR + 1) * NB]);
        for round in (1..NR).rev() {
            // -- nocapture for figuring out which round failed.
            println!("round {:?}", round);
            assert_eq!(state, test_table[14 - round][ISTART]);

            inv_shift_rows(&mut state);
            assert_eq!(state, test_table[14 - round][IROW]);

            inv_sub_bytes(&mut state);
            assert_eq!(state, test_table[14 - round][IBOX]);

            assert_eq!(
                &w[round * NB..(round + 1) * NB],
                test_table[14 - round][IKEY]
            );
            add_round_key(&mut state, &w[round * NB..(round + 1) * NB]);
            assert_eq!(state, test_table[14 - round][IADD]);
        ...
        add_round_key(&mut state, &w[0..NB]);
        assert_eq!(&w[0..NB], test_table[14][3]);
    ...
    fn init_inv_debug_table() -> [[[u32; 4]; 5]; 15] {
        [
            [
            ...
            [
                // Round 14 is a special case in that it includes the final
                // output and does not have a mix columns.
                hex_to_u32arr("6353e08c0960e104cd70b751bacad0e7"), // Start
                hex_to_u32arr("63cab7040953d051cd60e0e7ba70e18c"), // InvRow
                hex_to_u32arr("00102030405060708090a0b0c0d0e0f0"), // InvSub
                hex_to_u32arr("000102030405060708090a0b0c0d0e0f"), // KeySched
                hex_to_u32arr("00112233445566778899aabbccddeeff"), // Final
            ],
        ]
    }
