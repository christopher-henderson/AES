...
mod tests {
    ...
    fn decrypt_test() {
    	...
    	assert_eq!(ciphertext, hex_to_u32arr("00112233445566778899aabbccddeeff"));
    }
    ...
    fn inv_debug() {
		...
		for round in (1..NR).rev() {
			...
			assert_eq!(&w[round*NB..(round+1)*NB], test_table[14-round][IKEY]);
			add_round_key(&mut state, &w[round*NB..(round+1)*NB]);
    ...
    fn whole_file() {
    	...
    	let f: std::vec::Vec<u8> = fs::read("/Users/chris/Documents/personal/rust/projects/AES/Cargo.toml").unwrap();
    	let f: std::vec::Vec<u32> = f[..].chunks(4).map(|chunk| {
    		let mut ret = 0;
    	...
    	fs::write("/Users/chris/Documents/personal/rust/projects/AES/data/please", output).unwrap();
    }
}
